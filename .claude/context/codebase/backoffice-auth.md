...
| ../../shared/auth/partner-scope.ts | Partner 데이터 스코핑 유틸 | PartnerScope, extractPartnerScope(), withPartnerScope() |
...
### PartnerScope 데이터 스코핑

1. `extractPartnerScope(ctx)` → ctx.admin에서 authType/partnerId 기반으로 PartnerScope 추출
2. `withPartnerScope(ctx, input)` → ADMIN이면 그대로 반환, Partner면 partnerScope 주입
3. `PartnerScope` 타입: `{ authType: 'ADMIN' } | { authType: 'BRAND', brandId } | { authType: 'INFLUENCER', influencerId }`
4. tRPC Router에서 input에 주입 → Controller → Service에서 QueryBuilder WHERE 조건으로 변환
