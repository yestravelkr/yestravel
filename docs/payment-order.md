...
**이유:**
PG사의 거래 내역과 데이터를 정확히 일치시켜야 합니다. PG사에서는 각 결제/취소를 개별 거래로 관리하므로, 우리도 각 거래마다 Purchase를 생성해야 합니다.

**예시:**
```
시나리오:
1. 최초 주문: Purchase #1 (50,000원 결제)
2. 부분 취소: Purchase #2 (20,000원 취소)
3. 추가 옵션: Purchase #3 (10,000원 결제)

결과:
- Order는 1개
- Purchase는 3개 (PG사 거래 내역과 1:1 매칭)
...
**관계:**
- `1:N` ProductOption (품목당 여러 옵션 가능)
- `1:N` OrderOption (주문 시 선택된 품목 추적)
...
### ProductOption (품목 옵션)
품목의 선택 가능한 옵션을 정의합니다.

**관계:**
- `N:1` Product (특정 품목에 속함)
...
**관계:**
- `1:N` OrderOption (주문 내 여러 품목 포함 가능)
...
### OrderOption (주문 옵션)
주문에 포함된 구체적인 품목과 옵션 정보입니다.

**관계:**
- `N:1` Order (특정 주문에 속함)
- `N:1` Product (어떤 품목인지 참조)
