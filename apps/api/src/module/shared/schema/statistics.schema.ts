...
export const StatisticsDataItemSchema = z.object({
...
export function createStatisticsSchema(keys: string[]) {
  const dynamicProperties = keys.reduce((acc, key) => {
...
export function createStatisticsSchemaFromLegends(legends: { name: string; key: string }[]) {
