...
/**
 * 필터 칩에 표시할 옵션 라벨을 생성합니다.
 * 히스토리에 연결된 optionId 목록을 기반으로 라벨을 만듭니다.
 */
function getOptionChipLabel(
  optionIds: number[],
  optionsMap: Map<number, string>,
): string {
  if (optionIds.length === 0) return '';
  const firstName = optionsMap.get(optionIds[0]) ?? '알 수 없는 옵션';
  if (optionIds.length === 1) return firstName;
  return `${firstName} 외 ${optionIds.length - 1}건`;
}
